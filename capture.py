...
def save_face(emotion):
    ...
    for i in facedict.keys():
        path, dirs, files = next(os.walk("data/train/%s" %emotion))
        file_count = len(files)+1
        cv2.imwrite("data/train/%s/%s.jpg" %(emotion, (file_count)), facedict[i])
    facedict.clear()
...
def checkForFolders(emotions):
    for emotion in emotions:
        if os.path.exists("data/train/%s" %emotion):
            pass
        else:
            os.makedirs("data/train/%s" %emotion)
